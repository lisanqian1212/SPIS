# -*- coding: utf-8 -*-
"""
Created on Fri Jun 18 16:02:42 2023

@author: modified by lsq
"""
#%%
import sys
import util
...
def model_predict(x0, betas, t, *, status='train', transpose=True):
    with torch.no_grad():
        ...
        if status == 'train':
            xt = torch.sqrt(1-a_t)*e+torch.sqrt(a_t)*x0  #扩散后的不清晰图像Xt
        elif status == 'test':
            xt = x0
        else:
...
# model_dir = 'model/'
device = torch.device("cuda:3" if( torch.cuda.is_available() and gpu>0 ) else "cpu")
    ...
    with tqdm(total=len(values)) as pbar:
        for step, x in enumerate(train_data):
            ...
            if step % (len(train_data)-1) == 0 and step != 0:
                ...
                # plt.subplot(1,3,1),plt.imshow(x0,cmap='gray'),plt.axis('off'),plt.title('noisy')
                # plt.subplot(1,3,2),plt.imshow(x_pred,cmap='gray'),plt.axis('off'),plt.title('sample')
                # plt.subplot(1,3,3),plt.imshow(xt,cmap='gray'),plt.axis('off'),plt.title('t={}'.format(timestep))
                # plt.show()
                # plt.close()
                
        if epoch % 10 == 0 and epoch != 0:
            torch.save(model.state_dict(), os.path.join('model/logfundusS001/','checkpoint_{}.pth'.format(epoch)) )
